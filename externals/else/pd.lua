...
local function register_lua_class(name)
    local class = pd.Class:new():register(name)
    ...
    function class:dsp(sample_rate, block_size)
    ...
    function class:dispatch(inlet, sel, atoms)
        if sel == "load" then
            self:dofile(atoms[1])
            return
        end
...
register_lua_class("lua")
register_lua_class("else/lua")
